...
describes what users get. Pick a format with `-o`, or let vietnamtravel choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
vietnamtravel <command> -o table   # aligned columns for reading
vietnamtravel <command> -o jsonl   # one JSON object per line, for piping
vietnamtravel <command> -o json    # a single JSON array
vietnamtravel <command> -o csv     # spreadsheet friendly
vietnamtravel <command> -o tsv     # tab-separated
vietnamtravel <command> -o url     # just the URL column
vietnamtravel <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
vietnamtravel <command> --fields id,title,url
```
...
```bash
vietnamtravel <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
vietnamtravel <command>            # a table, because this is a terminal
vietnamtravel <command> | wc -l    # JSONL, because this is a pipe
```
